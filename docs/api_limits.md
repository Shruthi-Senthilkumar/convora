...
* Confirmed working via direct TTS call (eleven\_turbo\_v2\_5, Rachel voice),
2026-08-06

## Key acquisition status

All three keys acquired, scoped appropriately, and verified working via
eval/smoke\_test\_keys.py as of 2026-08-06.

## ElevenLabs - known limitation (2026-08-14)

Free tier no longer allows API access to library voices (e.g. Rachel) -

returns 402 payment\_required. Key correctly scoped to Text-to-Speech

only (Voices: No Access) per Phase 0 security setup. Needs resolution

before Phase 5 (TTS integration) - either find a genuinely free-tier

voice, or accept a paid plan at that point. Not blocking Phase 1-4.
